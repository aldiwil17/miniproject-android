...
public class Graph {
    ...
    public void modifyNode(Object data, int position){
        Node currentNode = getNode(position);
        final List<Node> edges = successorsOf(currentNode);
        this.nodes.remove(currentNode);
        currentNode.data = data;
        addNode(currentNode);
        for (Node destination : edges) {
            addEdge(currentNode, destination);
        }

        for (NodeObserver observer : observers) {
            observer.notifyDataChanged(currentNode);
        }
    }

    public void modifyNode(Object data, Node currentNode){
        final List<Node> edges = successorsOf(currentNode);
        this.nodes.remove(currentNode);
        currentNode.data = data;
        addNode(currentNode);
    ...
    public boolean removeNodeOnly(@NonNull Node node) {
